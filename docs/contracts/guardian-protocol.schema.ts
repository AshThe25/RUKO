/**
 * Ruko shared contracts — the guardian WIRE protocol.
 * Contract version: contracts-v1.2 (additive)
 *
 * Owner: Puneesh. Written on `feature/puneesh-android-guardian` and preserved
 * verbatim when that branch was rebased onto main.
 *
 * This is NOT a rival to `guardian.schema.ts` — the two are complementary and
 * both are needed:
 *
 *   guardian.schema.ts           what the PHONE sees: GuardianChannel,
 *                                GuardianAlert, GuardianDecision. The mobile
 *                                app codes against this and nothing else.
 *   guardian-protocol.schema.ts  what goes over the WIRE: REST payloads for
 *                                registration and pairing, the WebSocket
 *                                envelopes, heartbeats and relay errors. The
 *                                FastAPI relay and the Office Kit code against
 *                                this, and `backend/tests/test_contract_parity.py`
 *                                asserts the Python mirror has not drifted.
 *
 * Deliberately NOT re-exported from `index.ts`: it declares its own
 * `InferenceBackend`, which would collide with the one in
 * `conversation.schema.ts`. Reconciling those two is part of the open contract
 * reconciliation; until then, import this file by path.
 */
