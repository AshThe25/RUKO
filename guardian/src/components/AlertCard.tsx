  ...
  return (
    <article className={alert.band === 'CRITICAL' && !acked ? 'card card-critical' : 'card'}>
      <div className="alert-top">
        ...
        <span className="chip chip-muted">{KIND_LABEL[alert.kind] ?? alert.kind}</span>
      ...
      <div className="alert-payee">
        {alert.payee_label ? `to ${alert.payee_label}` : 'Recipient not recorded'}
      </div>
      ...
      <div className="alert-foot">
        <span className="small">
          {acked ? 'Handled. No further action needed.' : 'Waiting for someone to confirm they have seen this.'}
        </span>
        <button
